...
def _force_priority_prediction_keys(
    prediction_keys: List[str],
    priority_models: List[str],
) -> List[str]:
    """Prefer strong baseline/deep models if present; else keep all keys."""
    out: List[str] = []
    for model in priority_models:
        ...
        for k in prediction_keys:
            if k.endswith(suffix) and k not in out:
                out.append(k)
    if out:
        # keep any remaining keys after the priority models
        for k in prediction_keys:
            if k not in out:
                out.append(k)
        return out
...
def _soft_weighted_ensemble_from_val_metrics(
    ...
    """Fit soft weights on validation metrics and apply to prediction dictionary.

    selected_keys are keys like cand0_DLinear.
    """
...
def run_funnel_single_slice(
    ...
    # =====================================================================
    # Ensemble: fit soft weights on validation holdout, apply to held-out set
    # =====================================================================
    ...
    # Per-model metrics on held-out set
    ...
    # Candidate set: prioritize known strong baselines/deep models, but keep others available.
    ...
    # Fallback: if soft ensemble failed, use best single by combined score on held-out set
...
def _aggregate_slice_results(
    ...
    """Plain mean across slices, including benchmark metrics when available."""
