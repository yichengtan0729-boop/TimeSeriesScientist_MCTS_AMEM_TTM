...
def _metrics_from_pred(y_true: np.ndarray, y_pred: np.ndarray) -> Dict[str, float]:
    """Raw-scale metrics: MSE, MAE, robust MAPE."""
...
def _benchmark_metrics_from_pred(
    ...
    """
    Benchmark-aligned metrics:
    normalize y_true / y_pred using TRAIN split mean/std only,
    then compute MSE / MAE in normalized space.
    If train_series is unavailable, caller may pass validation series as fallback.
    """
    ...
    if len(train_series) == 0 or n == 0:
        return {"benchmark_mse": float("nan"), "benchmark_mae": float("nan")}
...
def _all_metrics_from_pred(
    ...
    out["selection_score"] = 0.5 * out["mae"] + 0.5 * out["benchmark_mae"]
    return out
...
def _default_apply_preprocess(data: Any, params: Dict[str, Any]) -> Any:
    ...
    Key idea: any global statistics (outlier thresholds, normalization params) must be
    computed on the *fit segment* only, then applied to the full slice.

    - Missing-value handling is applied to the full slice (ETT has none; kept for completeness).
    - Outlier thresholds and normalization parameters are fit on vals[:fit_len].
    - Stationarity transform is applied after normalization (as before), and we keep
      metadata needed for inverse-transform later.

    params supports:
        - _fit_len: int, number of leading points to use for fitting statistics.
    """
    ...
    # 1) Missing value strategy (kept; typically no NaNs in ETT)
    ...
    # 2) Outlier detect + handle (fit thresholds on fit segment, apply to full slice)
    ...
    if outlier_detect and outlier_detect != "none" and outlier_handle and outlier_handle != "none" and n > 0:
        # Compute bounds on fit segment only
        low, high = None, None
        ...
        if low is not None and high is not None and low < high:
            ...
            if np.any(outlier_mask):
                if outlier_handle == "clip":
                    ...
                    # interpolate only over the full slice but using neighbors; no future-to-past leakage
                    s = pd.Series(vals)
                    s[outlier_mask] = np.nan
                    vals = s.interpolate(limit_direction="both").to_numpy(dtype=float)
                elif outlier_handle == "smooth":
                    # simple smoothing: replace outliers with rolling median (past-only) using a small window
                    s = pd.Series(vals)
    ...
    # Keep original-scale copy (after missing/outlier handling, before normalization/stationarity)
    value_original = vals.copy()

    # 3) Normalization (fit on fit segment only, apply to full slice)
    ...
    # 4) Stationarity (applied AFTER normalization). Store pre-stat series for inverse.
...
def _make_select_models_fn(
    ...
    The chosen model is constrained by available_models if provided.
    """
...
def _soft_anchor_weights(
    ...
    """
    Softer ensemble weights from validation MAPE.
    Lower MAPE => higher weight.
    """
    if not val_aligned:
        return {}

    keys = list(val_aligned.keys())
    scores = []
    for k in keys:
        pred = np.asarray(val_aligned[k]).flatten()
        scores.append(_metrics_from_pred(val_y, pred)["mape"])
...
def run_funnel_single_slice(
    ...
    # Benchmark metrics prefer train split if available; otherwise gracefully
    # fall back to validation split so legacy slices still run.
    ...
    for i, cand in enumerate(best_candidates):
        try:
            ...
            if cand_l1:
                # Fit any global stats (outlier thresholds / normalization) on TRAIN ONLY to avoid leakage
    ...
    # 用 MAPE 来拟合权重（不是 MSE），但仍然加 safety 防止在 val 上变差
    ...
    if predictions_val:
        ...
        # 你的 ensemble_agent.run 返回的是 dict（不是 tuple），所以这里不要写成 _, info
        ensemble_info = ensemble_agent.run(val_aligned, val_y)
        if ensemble_info is None:
            ensemble_info = {"weights": {}, "selected_models": [], "method": ensemble_method}
    else:
        ensemble_info = {"weights": {}, "selected_models": [], "method": ensemble_method}

    # ---------------- SAFETY: never worse than best single on VAL (MAPE) ----------------
    best_key = None
    second_key = None
    best_mape = float("inf")
    second_mape = float("inf")

    # 找 val 上最好的 & 第二好的单模型（按 MAPE）
    for _k, _pred in val_aligned.items():
        _m = _metrics_from_pred(val_y, np.asarray(_pred).flatten())["mape"]
        if _m < best_mape:
            second_mape, second_key = best_mape, best_key
            best_mape, best_key = _m, _k
        elif _m < second_mape:
            second_mape, second_key = _m, _k

    use_fallback = False
    w = ensemble_info.get("weights", {}) if isinstance(ensemble_info, dict) else {}

    if (not w) or (best_key is None):
        use_fallback = True
    else:
    ...
    # ---------------- SOFT ANCHOR on VAL ----------------
    try:
        soft_weights = _soft_anchor_weights(
            val_aligned=val_aligned,
            val_y=val_y,
            temperature=float(config.get("soft_ensemble_temperature", 0.5)),
            uniform_mix=float(config.get("soft_ensemble_uniform_mix", 0.10)),
            max_weight=float(config.get("soft_ensemble_max_weight", 0.80)),
        )

        if soft_weights:
            soft_info = {
                "method": "soft_anchor_val",
                "selected_models": list(soft_weights.keys()),
                "weights": soft_weights,
            }
            _soft_val = ensemble_agent.apply_weights(val_aligned, soft_info)
            _soft_val_mape = _metrics_from_pred(val_y, np.asarray(_soft_val).flatten())["mape"]

            # only accept if not worse than best single
            if best_key is not None and _soft_val_mape <= best_mape:
                ensemble_info = soft_info
    ...
    if predictions_dict:
        ...
        if best_single_key is not None and ens_mape_tmp > best_single_mape:
            ...
            if second_key is not None and second_arr is not None:
                ...
                test_metrics["ensemble"] = _all_metrics_from_pred(train_series_for_metrics, y_true_arr, ensemble_pred_arr)
...
def _aggregate_slice_results(
    ...
    for key in sorted(all_keys):
        ...
        bmse = np.asarray([m.get("benchmark_mse", float("nan")) for m in key_metrics], dtype=float)
        bmae = np.asarray([m.get("benchmark_mae", float("nan")) for m in key_metrics], dtype=float)

        aggregated_metrics[key] = {
            "mse": float(np.mean(mses)),
            "mae": float(np.mean(maes)),
            "mape": float(np.mean(mapes)),
            "benchmark_mse": float(np.mean(bmse)),
            "benchmark_mae": float(np.mean(bmae)),
        }
